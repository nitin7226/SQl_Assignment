...
select physician.name,undergoes.physician,trained_in.* from physician 
join undergoes on undergoes.physician = physician.employeeid
join trained_in on physician.employeeid = trained_in.physician 
where trained_in.physician= undergoes.physician and treatment = 1 and procedures=treatment;
...
select physician.name physician_name ,undergoes.dateundergoes,trained_in.certificationexpires from physician 
join undergoes on undergoes.physician = physician.employeeid
join trained_in on physician.employeeid = trained_in.physician
where undergoes.dateundergoes>trained_in.certificationexpires;
...
-- 8.5 Obtain the information foor appointments 
-- where a patient met with a physician other than his/her primary care physician. 
...
Physician ,
nurse
LEFT JOIN appointment  ON Appointment.PrepNurse = Nurse.Employeeid
...
FROM
	Undergoes 
WHERE
	Patient <> ( SELECT Patient FROM Stay  WHERE Undergoes.Stay = Stay.StayID );
...
WHERE
	...
	FROM
		...
		Room 
	WHERE
		On_call.BlockFloor = Room.BlockFloor 
